...
addEventHandler("onClientHouseSystemMenueUpdate", getRootElement(), function(string, value, int)
	...
	if(string == "LOCKED") then
		if(value == 0) then
			guiSetText(Knopf2[6], "Закрыть дом")
		else
			guiSetText(Knopf2[6], "Открыть дом")
		end
	elseif(string == "MONEY") then
		guiSetText(Label2[3], "Крышек:\n"..value)
	-- elseif(string == "WEAPON") then
		-- if(value == 1) then
			-- if(type(int) == "string") then
				-- guiSetText(Label2[6], "Слот 1: "..getWeaponNameFromID(gettok(int, 1, ",")).."\nПатрон: "..gettok(int, 2, ","))
			-- else
				-- guiSetText(Label2[6], "Слот 1: -\nПатрон: -")
			-- end
		-- elseif(value == 2) then
			-- if(type(int) == "string") then
				-- guiSetText(Label2[7], "Слот 2: "..getWeaponNameFromID(gettok(int, 1, ",")).."\nПатрон: "..gettok(int, 2, ","))
			-- else
				-- guiSetText(Label2[7], "Слот 2: -\nПатрон: -")
			-- end
		-- elseif(value == 3) then
			-- if(type(int) == "string") then
				-- guiSetText(Label2[9], "Слот 3: "..getWeaponNameFromID(gettok(int, 1, ",")).."\nПатрон: "..gettok(int, 2, ","))
			-- else
				-- guiSetText(Label2[9], "Слот 3: -\nПатрон: -")
			-- end
		-- end
	-- elseif(string == "RENTABLE") then
		-- if(value == true) then
			-- guiSetText(Knopf2[9], "Запретить аренду")
		-- else
			-- guiSetText(Knopf2[9], "Разрешить аренду")
		-- end
	-- elseif(string == "RENTALPRICE") then
		-- guiSetText(Label2[15], "Стоимость аренды: $"..value)
...
addEventHandler("onClientHouseSystemInfoMenueOpen", gMe, function(owner, x, y, z, price, locked, rentable, rentalprice, id, doingrent, isrentedin, playerid, admin)
	if(Guivar3 == 1) then
	...
	if(tonumber(locked) == 1) then lockedstate = "Да" end
	
	local X, Y, Width, Height = getMiddleGuiPosition(270,280)
	Fenster3[1] = guiCreateWindow(X, Y, Width, Height,"Houseinformation",false)
	Label3[1] = guiCreateLabel(12,43,253,15,"ID дома: "..id,false,Fenster3[1])
	guiLabelSetHorizontalAlign(Label3[1],"center",false)
	guiSetFont(Label3[1],"default-bold-small")
	Label3[2] = guiCreateLabel(10,22,257,15,"Информация:",false,Fenster3[1])
	...
	Label3[4] = guiCreateLabel(12,61,251,18,"Локация: "..getZoneName(x, y, z, false),false,Fenster3[1])
	guiLabelSetHorizontalAlign(Label3[4],"center",false)
	guiSetFont(Label3[4],"default-bold-small")
	Label3[5] = guiCreateLabel(12,81,250,14,"Владелец: "..owner,false,Fenster3[1])
	guiLabelSetHorizontalAlign(Label3[5],"center",false)
	guiSetFont(Label3[5],"default-bold-small")
	Label3[6] = guiCreateLabel(12,100,252,15,"Цена: "..price.." крышек",false,Fenster3[1])
	guiLabelSetHorizontalAlign(Label3[6],"center",false)
	guiSetFont(Label3[6],"default-bold-small")
	-- Label3[7] = guiCreateLabel(12,118,250,14,"Аренда: "..rentstate,false,Fenster3[1])
	-- guiLabelSetHorizontalAlign(Label3[7],"center",false)
	-- guiSetFont(Label3[7],"default-bold-small")
	-- Label3[8] = guiCreateLabel(12,137,247,14,"Цена аренды: $"..rentalprice,false,Fenster3[1])
	-- guiLabelSetHorizontalAlign(Label3[8],"center",false)
	-- guiSetFont(Label3[8],"default-bold-small")
	Label3[9] = guiCreateLabel(12,155,247,12,"Закрыт: "..lockedstate,false,Fenster3[1])
	guiLabelSetHorizontalAlign(Label3[9],"center",false)
	guiSetFont(Label3[9],"default-bold-small")
	Knopf3[1] = guiCreateButton(9,175,124,32,"Купить дом",false,Fenster3[1])
	-- Knopf3[2] = guiCreateButton(137,175,124,32,"Арендовать дом",false,Fenster3[1])
	Knopf3[3] = guiCreateButton(9,210,124,32,"Закрыть меню",false,Fenster3[1])
	Knopf3[4] = guiCreateButton(137,209,124,32,"Войти",false,Fenster3[1])
	Knopf3[5] = guiCreateButton(9,246,252,25,"Использовать заколку",false,Fenster3[1])
	guiSetEnabled ( Knopf3[5], false )
	...
	-- ENTER BUTTON --
		--outputChatBox ( tostring ( admin ) )
	if(tonumber(getElementData ( gMe, "playerid" )) ~= playerid and not admin) then
		if(isrentedin == true) then
		
		else
			if(tonumber(locked) == 1) then
				guiSetEnabled(Knopf3[4], false)
				guiSetEnabled ( Knopf3[5], true )
			end
		end
	end
	
	-- RENT HOUSE BUTTON --
	if isElement ( Knopf3[2] ) then
		if(tonumber(getElementData ( gMe, "playerid" )) == playerd) then
			...
			if(isrentedin == true) then
				...
				guiSetText(Knopf3[2], "Покинуть дом")
			end
		end
	end
	...
	if(owner ~= "no-one") then
		...
		if(playerid == tonumber(getElementData ( gMe, "playerid" ))) then
			guiSetEnabled(Knopf3[1], true)
			guiSetText(Knopf3[1], "Продать дом")
		end
	end
	-- EVENT HANDLERS --
	...
	addEventHandler("onClientGUIClick", Knopf3[5], function()
		guiSetEnabled (Fenster3[1],false)
		askPasswordWindowC()
	end, false)
	...
	addEventHandler("onClientGUIClick", Knopf3[4], function()
		...
		showCursor(false)
		destroyElement(Fenster3[1])
		triggerServerEvent("onHouseSystemInfoEnter", gMe, id,withpin)
		withpin = false
	end, false)
...
function askPasswordWindowC ()
	if not isElement ( activateWindow ) then
		activateWindow = guiCreateWindow ( xs/2-150, ys/2-75, 305, 160, "Взломать замок от двери", false )
		guiSetAlpha ( activateWindow, 255 )
		
		exports.inventory:removePlayerItem(126, 1 )
		local tempmax = maxnum
		if getElementData ( localPlayer, "perk34" ) then tempmax = math.floor(tempmax/2) end
		pincode = math.random ( 1, tempmax )
		
		descrLabel = guiCreateLabel ( 10, 25, 285, 60, "Введите число от 1 до "..tempmax..".", false, activateWindow )
...
function closeActivation ()
	...
	destroyElement ( activateWindow )
	guiSetEnabled (Fenster3[1],true)
end

function startActivation ()
	if isElement ( codeEdit ) then
		...
		if text then
			if text == pincode then
				guiSetEnabled (Knopf3[4],true)
				guiSetEnabled (Knopf3[5],false)
				withpin = true
				return true 
			end
		end
		exports.pipboy:addInfoBox ("Не удалось взломать замок.", 2)
	end
end
...
addEventHandler("onClientHouseSystemMenueOpen", gMe, function(owner, locked, money, weap1, weap2, weap3, id, rentable, rent, tenands)
	if(locked == nil) then
		...
		Fenster2[1] = guiCreateWindow(X, Y, Width, Height, "Дом",false)
		Label2[1] = guiCreateLabel(149,23,81,14,"Оружие",false,Fenster2[1])
		...
		Label2[3] = guiCreateLabel(17,40,106,32,"Количество крышек:\n0",false,Fenster2[1])
		guiSetFont(Label2[3],"default-bold-small")
		Edit2[1] = guiCreateEdit(16,75,109,24,"",false,Fenster2[1])
		Knopf2[1] = guiCreateButton(16,105,110,29,"Положить",false,Fenster2[1])
		Knopf2[2] = guiCreateButton(16,136,110,29,"Снять",false,Fenster2[1])
		Knopf2[3] = guiCreateButton(15,175,110,29,"Закрыть меню",false,Fenster2[1])
		Label2[4] = guiCreateLabel(18,23,47,15,"Деньги",false,Fenster2[1])
		...
		Label2[6] = guiCreateLabel(142,50,139,33,"Слот 1: -\nПатрон: -",false,Fenster2[1])
		...
		Label2[7] = guiCreateLabel(139,95,139,33,"Слот 2: -\nПатрон: -",false,Fenster2[1])
		...
		Label2[9] = guiCreateLabel(137,138,139,33,"Слот 3: -\nПатрон: -",false,Fenster2[1])
		...
		Radio2[1] = guiCreateRadioButton(303,27,108,21,"Слот оружия 1",false,Fenster2[1])
		guiSetFont(Radio2[1],"default-bold-small")
		Radio2[2] = guiCreateRadioButton(303,47,108,21,"Слот оружия 2",false,Fenster2[1])
		guiSetFont(Radio2[2],"default-bold-small")
		Radio2[3] = guiCreateRadioButton(303,67,108,21,"Слот оружия 3",false,Fenster2[1])
		guiRadioButtonSetSelected(Radio2[3],true)
		guiSetFont(Radio2[3],"default-bold-small")
		Knopf2[4] = guiCreateButton(301,104,107,30,"Положить оружие",false,Fenster2[1])
		Knopf2[5] = guiCreateButton(301,142,107,30,"Взять оружие",false,Fenster2[1])
		Knopf2[6] = guiCreateButton(301,175,107,29,"Закрыть дом",false,Fenster2[1])
		Label2[12] = guiCreateLabel(412,22,10,189,"|\n|\n|\n|\n|\n|\n|\n|\n|\n|\n|\n|\n|\n|\n|\n|\n|\n|\n|\n|\n|\n",false,Fenster2[1])
		guiLabelSetColor(Label2[12],0, 255, 0)
		guiSetFont(Label2[12],"default-small")
		-- Label2[13] = guiCreateLabel(424,24,137,15,"Аренда",false,Fenster2[1])
		...
		-- Label2[15] = guiCreateLabel(424,41,137,15,"Стоимость аренды: $0",false,Fenster2[1])
		-- guiSetFont(Label2[15],"default-bold-small")
		-- Edit2[2] = guiCreateEdit(441,58,75,24,"",false,Fenster2[1])
		-- Knopf2[7] = guiCreateButton(424,88,114,27,"Установить цену",false,Fenster2[1])
		...
		-- guiGridListAddColumn(Grid2[1],"Арендаторы",0.8)
		-- Knopf2[8] = guiCreateButton(422,171,115,30,"Убрать арендатора",false,Fenster2[1])
		-- Checkbox2[1] = guiCreateCheckBox(425,116,111,17,"Вы уверены?",false,false,Fenster2[1])
		-- guiSetFont(Checkbox2[1],"default-bold-small")
		-- Knopf2[9] = guiCreateButton(423,137,115,30,"Включить аренду",false,Fenster2[1])
		-- SET THE SETTINGS --
		-- TENANDS --
		guiGridListClear(Grid2[1])
		...
		-- if(rentable == 1) then
			-- guiSetText(Knopf2[9], "Отключить аренду")
		-- end
		-- RENTALPRICE
		--guiSetText(Label2[15], "Стоимость аренды: $"..rent)
		...
		-- MONEY --
		guiSetText(Label2[3], "Количество крышек:\n"..money)
		...
		-- if(tonumber(weap1) == nil) then
			...
			-- if(weapon) and (ammo) then
				-- guiSetText(Label2[6], "Слот 1: "..getWeaponNameFromID(weapon).."\nПатрон: "..ammo)
			-- end
		-- end
		-- if(tonumber(weap2) == nil) then
			-- local weapon, ammo = gettok(weap2, 1, ","), gettok(weap2, 2, ",")
			-- if(weapon) and (ammo) then
				-- guiSetText(Label2[7], "Слот 2: "..getWeaponNameFromID(weapon).."\nПатрон: "..ammo)
			-- end
		-- end
		-- if(tonumber(weap3) == nil) then
			-- local weapon, ammo = gettok(weap3, 1, ","), gettok(weap3, 2, ",")
			-- if(weapon) and (ammo) then
				-- guiSetText(Label2[9], "Слот 3: "..getWeaponNameFromID(weapon).."\nПатрон: "..ammo)
			-- end
		-- end
		-- EVENT HANDLERS --
		...
		addEventHandler("onClientGUIClick", Knopf2[8], function()
			if(owner == false) then outputChatBox("Вы не владелец этого дома!", 255, 0, 0) return end
			local user = guiGridListGetItemText(Grid2[1], guiGridListGetSelectedItem(Grid2[1]), 1)
			if(user == "") or (user == " ") then return end
			if(guiCheckBoxGetSelected(Checkbox2[1]) == false) then outputChatBox("Определись для начала!", 255, 0, 0) return end
		...
		addEventHandler("onClientGUIClick", Knopf2[7], function()
			...
			if(value == nil) or(value < 1) or (value > 20000) then outputChatBox("Неверная сумма!", 255, 0, 0) return end
		...
		addEventHandler("onClientGUIClick", Knopf2[2], function()
			...
			if(value < 0)  then outputChatBox("Неверная сумма!", 255, 0, 0) return end
			value = math.floor(value)
		...
		addEventHandler("onClientGUIClick", Knopf2[1], function()
			...
			if(value == "") then return end
			value = tonumber(value)
			if(value < 0) or (value > getPlayerMoney(gMe)-1) then outputChatBox("Неверная сумма!", 255, 0, 0) return end
			value = math.floor(value)
				...
				if(isLineOfSightClear(x, y, z, x2, y2, z2, true, true, false, true)) then
					...
					if(sx) and (sy) then
						...
						if(distance < 20) then
							...
							local lockedstate = "открыт"
							if(tonumber(locked) == 1) then lockedstate = "закрыт" end
							...
							if(owner) and (price) and (locked) and (houseid) then
								if(owner ~= "no-one") then
									...
									if(rentable == 1) then
										dxDrawText("Дом: "..houseid.." ("..zone..")\nВладелец: "..owner.."\nЦена: "..price.." крышек\nМожно арендовать комнату. Стоимость: "..rentalprice.." крышек\nДом "..lockedstate..".", sx+2, sy+2, sx, sy, tocolor(0, 0, 0, 200), fontbig, "default-bold", "center")
										dxDrawText("Дом: "..houseid.." ("..zone..")\nВладелец: "..owner.."\nЦена: "..price.." крышек\nМожно арендовать комнату. Стоимость: "..rentalprice.." крышек\nДом "..lockedstate..".", sx+2, sy+2, sx, sy, tocolor(255, 255, 255, 200), fontbig, "default-bold", "center")
									else
										dxDrawText("Дом: "..houseid.." ("..zone..")\nВладелец: "..owner.."\nЦена: "..price.." крышек\nДом "..lockedstate..".", sx+2, sy+2, sx, sy, tocolor(0, 0, 0, 200), fontbig, "default-bold", "center")
										dxDrawText("Дом: "..houseid.." ("..zone..")\nВладелец: "..owner.."\nЦена: "..price.." крышек\nДом "..lockedstate..".", sx, sy, sx, sy, tocolor(255, 255, 255, 200), fontbig, "default-bold", "center")
									end
								else
									dxDrawText("Дом: "..houseid.." ("..zone..")\nВладелец: "..owner.."\nЦена: "..price.." крышек\nДля покупки дома, используйте /buyhouse\nДом "..lockedstate..".", sx+2, sy+2, sx, sy, tocolor(0, 0, 0, 200), fontbig, "default-bold", "center")
									dxDrawText("Дом: "..houseid.." ("..zone..")\nВладелец: "..owner.."\nЦена: "..price.." крышек\nДля покупки дома, используйте /buyhouse\nДом "..lockedstate..".", sx, sy, sx, sy, tocolor(255, 255, 255, 200), fontbig, "default-bold", "center")
								end
							end
						end
