function makeNPCWalkToPos(npc,x,y,z,maxtime)
	...
	if move and x and y and z and rot and type(x) == "number" and type(y) == "number" and type(z) == "number" and type(rot) == "number" then
...
function makeNPCDriveToPos(npc,x,y,z,maxtime)
	...
	if dist < drive_dist then
		...
		local newtask = getElementData(npc,"npc_hlc:task."..(thistask+1))
		if newtask then
			if newtask[1] == "driveAroundBend" then
				if getDistanceBetweenPoints3D (px,py,pz,newtask[4],newtask[5],newtask[6]) > drive_dist then
					setElementData ( npc, "npc_hlc:task."..thistask, {"driveToPos",newtask[4],newtask[5],newtask[6],5} ) 
					return maxtime
				end
			end
		end
	...
	outputDebugString("move "..tostring(move))
	if move then
