...
function makeNPCShootAtElement(npc,target)
	...
	if tgtype == "ped" or tgtype == "player" then
		...
		if vehicle then
			...
			local t_Matrix = getElementMatrix (npc)
			local int_RayX = t_Matrix[2][1] + t_Matrix[4][1]
			local int_RayY = t_Matrix[2][2] + t_Matrix[4][2]
			local int_RayZ = t_Matrix[2][3] + t_Matrix[4][3]
			for i = 1, #bones-1 do
				if isLineOfSightClear(int_RayX,int_RayY,int_RayZ,x,y,z,true,true,true,true,true,false,false,target) then
					break
				else
					x,y,z = getPedBonePosition(target,bones[i+1])
				end
			end
		end
	end
...
function makeNPCDriveToPos(npc,x,y,z)
	...
	do
		...
		if not isLineOfSightClear(vx,vy,vz,vx+dx,vy+dy,vz+dz,false,true,false,false,true,false,false,car) then
			speed = 0
		else
			speed = getNPCDriveSpeed(npc)*math.sin(math.pi*0.5-math.atan(math.abs(rx/ry))*0.75)
		end
	end
	-- if getElementModel ( car ) == 427 then
		-- outputChatBox ("car: "..tostring(getElementModel(car)).." speed: "..speed.." "..tostring(vry < speed).." "..tostring(vry > speed*1.1))
	-- end
