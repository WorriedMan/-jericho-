...
function cycleNPCs()
	if getElementData ( root, "dayz:relooting" ) then return true end
	local streamed_npcs = {}
...
function npcJump ()
	if isElement(source) then
		...
		setTimer ( function (ped) if isElement ( ped ) then setPedControlState ( ped, "jump", false) end end, 800, 1, source )
	end
end
	
addEvent( "npc_hlc:npcJump", true )
addEventHandler( "npc_hlc:npcJump", getRootElement(), npcJump )
