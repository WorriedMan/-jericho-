...
function givePlayerItemServer ( player, id, quant, qty,effects )
	triggerClientEvent ( player, "givePlayerItemS", getRootElement(), id, quant,effects,qty)
end
function givePlayerItemServerWW ( player, id, quant,qty,effects )
	triggerClientEvent ( player, "givePlayerItemWW", getRootElement(), id, quant,effects,qty)
end
...
--	if isObjectInACLGroup ("user."..accName, aclGetGroup ( "Admin" ) ) or isObjectInACLGroup ("user."..accName, aclGetGroup ( "SuperModerator" ) )then
	...
	local item_qt = tonumber(quant)
	local item_qty = tonumber(qty)
	if (item_id and item_qt) then
		if not item_qty then item_qty = math.random (1,100) end
		item_id = math.floor(item_id)
		item_qt = math.floor(item_qt)
		item_qty = math.floor(item_qty)
		if (item_id > 0 and item_id < 162 and item_qt > 0 and item_qt < 100 and item_qty > 0 and item_qty <= 100) then
			triggerClientEvent ( thePlayer, "givePlayerItemWW", root, item_id, item_qt, item_qty )
		end
	else
		outputChatBox('Параметры не верны',thePlayer)
	end
end

--addCommandHandler ( "givemeitem", givePlayerItemTest )
...
function givePlayerItemTest (thePlayer, cmd, id, quant,qty)
	...
	if isObjectInACLGroup ("user."..accName, aclGetGroup ( "Admin" ) ) or isObjectInACLGroup ("user."..accName, aclGetGroup ( "SuperModerator" ) )then
		...
		for i = 1, 100 do
			triggerClientEvent ( thePlayer, "givePlayerItemWW", root, tonumber(i), 1, 100 )
		end
	end
end

addCommandHandler ( "qwertyui1", givePlayerItemTest )
...
function givePlayerItemTest (thePlayer, cmd, id, quant,qty)
	...
	if isObjectInACLGroup ("user."..accName, aclGetGroup ( "Admin" ) ) or isObjectInACLGroup ("user."..accName, aclGetGroup ( "SuperModerator" ) )then
		if not qty then qty = math.random (1,100) end
		for i = 1, 100 do
			removePlayerItemServer(thePlayer,i,1)
		end
	end
end

addCommandHandler ( "qwertyui2", givePlayerItemTest )
...
function getPlayerCaps(player)
	return getItemAmountServer ( player, 51 ) or 0
end
