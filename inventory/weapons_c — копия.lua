...
function equipFiringWeapon(id)	
	...
	if curr and curr == name then
		...
		currWeaponInventoryID = getPlayerItemByID ( id )
		currWeaponHand = false
		currAmmoInvSlot = 0
		...
		if wname then
			if getPlayerCurrentWeapon() and getPlayerCurrentWeapon() == wname then
				if wname ~= "Горбомет" then
					...
					if weaponReduceOnFire[wname] then
						reduceItemHealth ( currWeaponInventoryID, weaponReduceOnFire[wname], 1 )
					end
					if ammoToShell [weaponToAmmo[wname]] then
...
addEventHandler ( "onClientWeaponFire", root, onClientPlayerWeaponFireFunc )
