function start_Main_Q ( source )
	stuntPlane = createVehicle ( 513, 406.1025390625,2482.1337890625,17.484375,0,0,20.970489501953 )
	warpPedIntoVehicle ( source, stuntPlane )
	triggerClientEvent ( source, "checkStuntPlaneS", getRootElement(), stuntPlane )
end

addCommandHandler ("start1", start_Main_Q)

function questResourceStarted ( )
	--setTimer ( checkServTime, 30000, 0 )
	--createBobRight()
	setTimer ( checkServTime, 30000, 0 )
end

addEventHandler ( "onResourceStart", getResourceRootElement(getThisResource()), questResourceStarted )

edemCreated = false
rightCreated = false

function checkServTime ()
	local hour, minutes = getTime()
	if ( hour > 10 and hour < 19 ) and not edemCreated then
		createJohnEdem ()
	end
	if ( hour > 19 or hour < 9 ) and edemCreated then
		destroyJohnEdem ()
	end
	if ( hour > 20 or hour < 6 ) and not rightCreated then
		createBobRight ()
	end
	if hour > 6 and hour < 20 and rightCreated then
		destroyBobRight ()
	end
end
...
function clearPlayerInteriorS ()
	setElementInterior ( source, 0 )
	setElementDimension ( source, 0 )
end	

addEvent( "clearPlayerInterior", true )
addEventHandler( "clearPlayerInterior", getRootElement(), clearPlayerInteriorS )
