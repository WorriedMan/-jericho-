...
addEventHandler( "onClientPlayerEnterModShop", g_root, 
    function( vehicle, money, shopname )
        if getLocalPlayer() == source then
            ...
            upgradeChanged = { }
            newUpgrades = { }
            outputChatBox( "#FFFF00Добро пожаловать в #00FF00".. shopname .."!", 0,0,0, true )
...
addEventHandler( "onClientResourceStart", g_resRoot, function( )
        ...
        cartWnd = guiCreateWindow( 5, screenHeight - 320, 120, 150, "Корзина", false )
        ...
        costsLbl = guiCreateLabel( 10, 121, 110, 20, " Цена:  крышек", false, cartWnd )
        ...
        guiSetText( colorsWnd, "Выберите цвет для вашего автомобиля" )
        ...
        shopGUI.buttons[ "Exit" ] = guiCreateButton( 10, buttonHeight * btns + 8, shopWidth, buttonHeight - 5, "Выйти", false, shopGUI.wnd )
        ...
        upgradeGUI.gridList.grd = guiCreateGridList( 10, 23, upgWidth, 200, false, upgradeGUI.wnd )
            ...
            guiGridListAddColumn( upgradeGUI.gridList.grd, "Часть", 0.6 )
    ...
    if not hideSubToo then
        ...
        for k,v in ipairs( upgrades ) do
            if slotname == getVehicleUpgradeSlotName( v ) then
                ...
                guiGridListSetItemText( upgradeGUI.gridList.grd, row, 2, tostring( price ).." крышек", false, false )
        ...
        elseif type( showNewSub ) == "string" and showNewSub == "paintjob" then
            ...
            if getElementModel( moddingVeh ) ~= 575 then
                for i = 0, 2 do
                    ...
                    guiGridListSetItemText( upgradeGUI.gridList.grd, row, 2, "500 крышек", false, false )
                end
            else
                for i = 0, 1 do
                    local row = guiGridListAddRow( upgradeGUI.gridList.grd )
