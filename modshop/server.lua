...
modShops[ 1 ].marker = createMarker( -2723.7060, 217.2689, 299.6133, "cylinder", 3, 255, 0, 0, 100 )
...
modShops[ 2 ].marker = createMarker( 1990.6890, 2056.8046, 999.3844, "cylinder", 3, 255, 0, 0, 100 )
...
modShops[ 3 ].marker = createMarker( -2128.791015625,-116.654296875,34.324047088623, "cylinder", 3, 255, 0, 0, 100 )
modShops[ 3 ].name = "мастерская"
                ...
                if driver == player and not getVehicleInModShop( source ) then
                    for k,v in ipairs( modShops ) do
                        if modShops[ k ].marker == source and getElementType( vehicle ) == "vehicle" then
								...
								triggerClientEvent( driver, "onClientPlayerEnterModShop", player, vehicle, exports.inventory:getPlayerCaps(player), modShops[ k ].name )
                        end
                    end
                end
            end
...
addEventHandler( "modShop_playerLeaveModShop", getRootElement( ),
    function( vehicle, itemsCost, upgrades, colors, paintjob, shopName )
        local pMoney = exports.inventory:getPlayerCaps(source)
        if pMoney >= itemsCost then
            modTheVehicle( vehicle, upgrades, colors, paintjob, shopName )
			exports.inventory:removePlayerItemServer ( source, 51, itemsCost )
            triggerClientEvent( source, "modShop_moddingConfirmed", source )
        else
            outputChatBox( "#FF0000Проблема! #00FF00У вас только "..tostring( getPlayerMoney( source ) ).." крышек.#FFFFFF Удалите некоторые улучшения.", source, 0,0,0,true)
        end
    end
)
...
function modTheVehicle( vehicle, upgrades, colors, paintjob, shopName )
    if isElement( vehicle ) and getElementType( vehicle ) == 'vehicle' and type( upgrades ) == 'table' or getVehiclePaintjob( vehicle ) ~= paintjob then
		...
		if getElementData ( vehicle, "vehid" ) then
			exports.cars:saveVehicleUpgrades( vehicle )
		end
    end
end
