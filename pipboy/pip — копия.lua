...
function createMapBlip(button, state, cx, cy)
	if button == "left" then
		if isCursorInPosition(xPip+255, yPip+50, 350, 350) then
			...
			addMapIcon ( x-16, y-16, 0, 4, getPlayerName(localPlayer) )
		end
	end
end

function mapToWorldPosition(absoluteX,absoluteY)
	local xMap, yMap = xPip+255, yPip+50
	
	local relX = (absoluteX - xMap)/350
	local relY = (absoluteY - yMap)/350
...
function addMapIcon ( x, y, z, icon, text )
	tmpX = (x/100)-15
	tmpY = y/100
	local finX, finY = mapX+(x/20)+tmpX,mapY-(y/20)-tmpY
	local createdLabel = guiCreateLabel ( finX-8, finY-8, 27, 27, "", false )
	createdIcons = createdIcons+1
	setElementData ( createdLabel, "mapIconID", createdIcons )
	setElementData ( createdLabel, "mapAdditIcon", true )
	table.insert ( additIcons, { finX, finY, icon, text, x, y, z, createdLabel, createdIcons } )
--	addEventHandler ( "onClientGUIClick", createdLabel, mapIconClicked, false )
	return createdIcons
...
function renderPipBoy ()
	...
	if showMap == true then
		...
		dxDrawImageSection ( finX, finY, 12, 18, 324, 838, 22, 28, texture, -rotZ, 0, 0, color )
		...
		for i, icon in ipairs(additIcons) do
			if isCursorInPosition(icon[1]-20, icon[2]-20, 40, 40) and not iconFound then
				...
				if mapIconSelStage then
					...
					dxDrawImage ( icon[1]-12, icon[2]-12, 40, 40, t[map_iconsLinks[icon[3]]], 0, 0, 0, color )
				end
				...
				dxDrawText ( icon[4], textPosX, icon[2]-18, screenWidth, screenHeight, tocolor(201, 192, 187), 0.7, myFont )
			else
				if selMapIcon and selMapIcon == i and selectedType == 1 then
					selMapIcon = nil
					selectedType = nil
				end
				dxDrawImage ( icon[1]-8, icon[2]-8, 32, 32, t[map_iconsLinks[icon[3]]], 0, 0, 0, color )
			end
		end	
