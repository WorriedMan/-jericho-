...
function ani_start()
	...
	ani_aktualizuj(true)
	bindKey( "x", "down", "Animation open", "")
	bindKey( "-", "down", "Animation stop", "")
end
...
function ani_init()
	if not guiGetVisible(ani_window) then
		ani_otworz()
		outputChatBox ( 'Нажмите "-" для остановки текущей анимации' )
		outputChatBox ( 'Введите "/showhude" для скрытия HUD' )
	else
		ani_zamknij()
	end
end
